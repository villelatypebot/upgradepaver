...
export default function AdminPage() {
    ...
    }, [activeTab, isAuthenticated, dashboardLoaded, pricingLoaded, leadsLoaded, settingsLoaded]);
    ...
    const loadSettings = async () => {
        try { const r = await fetch('/api/admin/config'); const d = await r.json(); if (d.googleAiApiKey) setApiKey(d.googleAiApiKey); if (d.webhookUrl) setWebhookUrl(d.webhookUrl); setSettingsLoaded(true); } catch { }
    };
    ...
    const handleSaveConfig = async () => {
        try {
            const r = await fetch('/api/admin/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ googleAiApiKey: apiKey, webhookUrl }) });
                ...
                <nav className="flex-1 py-2">
                    {NAV_ITEMS.map(item => (
                        <button key={item.id} onClick={() => setActiveTab(item.id)}
                            className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition-all ${activeTab === item.id ? "bg-primary/10 text-primary border-r-2 border-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"
                                }`}>
                    ...
                    {activeTab === "settings" && (
                        <div className="space-y-4 md:space-y-6 max-w-2xl">
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-3">
                                    <div className="flex gap-2"><Input type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="Enter new API Key (Optional unless missing)" /><Button onClick={handleSaveConfig}>Save</Button></div>
                                    <p className="text-xs text-muted-foreground">Stored securely in database.</p>
                                </CardContent>
                            </Card>
                            <Card>
                                <CardHeader><CardTitle>Webhook URL (Zapier, Make, n8n)</CardTitle></CardHeader>
                                <CardContent className="space-y-3">
                                    <div className="flex gap-2"><Input type="url" value={webhookUrl} onChange={e => setWebhookUrl(e.target.value)} placeholder="https://..." /><Button onClick={handleSaveConfig}>Save</Button></div>
            ...
            <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t z-50 safe-area-pb">
                <div className="flex justify-around py-1">
                    {NAV_ITEMS.slice(0, 5).map(item => (
                        <button key={item.id} onClick={() => setActiveTab(item.id)}
                            className={`flex flex-col items-center gap-0.5 px-2 py-2 text-[10px] font-medium transition-colors min-w-0 ${activeTab === item.id ? "text-primary" : "text-muted-foreground"
                                }`}>
                    ...
                    <button onClick={() => setActiveTab("settings")}
                        className={`flex flex-col items-center gap-0.5 px-2 py-2 text-[10px] font-medium transition-colors min-w-0 ${activeTab === "settings" ? "text-primary" : "text-muted-foreground"
                            }`}>
